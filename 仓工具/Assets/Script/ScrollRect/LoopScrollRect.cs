...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected float paddingHorizontal, paddingVertical;
            ...
            get
            {
                ...
                if (content != null)
                {
                    ...
                    if (layout1 != null)
                    {
                        ...
                        paddingHorizontal = layout1.padding.left+ layout1.padding.right;
                        paddingVertical = layout1.padding.top+ layout1.padding.bottom;
                    }
                    m_GridLayout = content.GetComponent<GridLayoutGroup>();
                    if (m_GridLayout != null)
                    {
                        m_ContentSpacing = GetDimension(m_GridLayout.spacing);
                        paddingHorizontal = m_GridLayout.padding.left + m_GridLayout.padding.right;
                        paddingVertical = m_GridLayout.padding.top + m_GridLayout.padding.bottom;
                    }
                }
                return m_ContentSpacing;
            }
        ...
        public ScrollRectEvent onBeginDrag { get { return m_OnBeginDrag; } set { m_OnBeginDrag = value; } }

        [SerializeField]
        private ScrollRectEvent m_OnEndDrag = new ScrollRectEvent();
        public ScrollRectEvent onEndDrag { get { return m_OnEndDrag; } set { m_OnEndDrag = value; } }


        [SerializeField]
        private Action m_InertiaEndAction;
        private float m_ActionVelocity;
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            if (offset > 0)
            {
                ...
                {
                    itemTypeEnd -= 1;
                    itemTypeEnd = Math.Max(0, itemTypeEnd);
                }
                else
                {
                    itemTypeEnd += 1;
                    itemTypeEnd = Math.Min(totalCount, itemTypeEnd);
                }
            }
            ...
            if (directionSign == -1)
                sizeToFill = viewRect.rect.size.y + paddingHorizontal;
            else
                sizeToFill = viewRect.rect.size.x + paddingVertical;
        ...
        public void RefillCells(int offset = 0)
        {
            ...
            if (directionSign == -1)
                sizeToFill = viewRect.rect.size.y+paddingVertical;
            else
                sizeToFill = viewRect.rect.size.x+paddingHorizontal;
        ...
        public virtual void OnBeginDrag(PointerEventData eventData)
        {
            ...
            m_Dragging = true;
            m_OnBeginDrag?.Invoke(normalizedPosition);
        }
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            ...
            m_OnEndDrag?.Invoke(normalizedPosition);
            m_Dragging = false;
        }
            ...
            if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
            {
                ...
                for (int axis = 0; axis < 2; axis++)
                {
                    ...
                    else if (m_Inertia)
                    {
                        ...
                        if (m_InertiaEndAction != null)
                        {
                            if (m_Horizontal)
                            {
                                if (Mathf.Abs(m_Velocity[0]) < m_ActionVelocity)
                                {
                                    m_Velocity[0] = 0;
                                    m_InertiaEndAction.Invoke();
                                }
                            }
                            if (m_Vertical)
                            {
                                if (Mathf.Abs(m_Velocity[1]) < m_ActionVelocity)
                                {
                                    m_Velocity[1] = 0;
                                    m_InertiaEndAction.Invoke();
                                }
        ...
        public void SetInertiaAction(float Velocity, Action action)
        {
            m_InertiaEndAction = action;
            m_ActionVelocity = Velocity;
        }

        public void RemoveInertiaAction()
        {
            m_InertiaEndAction = null;
            m_ActionVelocity = 0;
        }
    }
}